        ...
        while j <= len(l):
            ...
            while int(p) != 0 and c <= delta:
                ...
                if int(benign[gamma]) == 1 and int(x[gamma]) == 0:
                    x[gamma] = 1  # Se solo l'applicativo benigno ha il permesso considerato, impongo il permesso anche all'apk maligno
                    c += 1
                    ...
                    # Apro, in lettura il file risultante dalla classificazione effettuata mediante Weka
            ...
            """
            Una volta al di fuori del while verrà verificata la condizione d'uscita:
