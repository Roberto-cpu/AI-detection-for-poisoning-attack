...
def k_means_cluster(df, na, m, n):
    ...
    mean = np.mean(df, axis=0)  # calcolo la media dei valori nel dataset
    std = np.std(df, axis=0)  # calcolo la deviazione standard dei valori nel dataset
...
def main():
    ...
    for var in range(len(numpy_array)):
        if numpy_array[var, permissions_number - 1] == 1:
            ...
            benigns.append(np.array(numpy_array[var, range(0, permissions_number)]))
    centers = k_means_cluster(benigns, numpy_array, apks_number, permissions_number)
    i = 0
    tau = 0
    ncluster = 3
    ...
    for i in range(len(malware)):
        ...
        for j in range(len(centers)):
            ...
            while count < ncluster:
                ...
                if int(c[gamma]) == 1 and int(x[gamma]) == 0 and int(a[gamma]) == 1:
                    ...
                    if mal_counter < len(predictions)-1:
                        mal_counter += 1
