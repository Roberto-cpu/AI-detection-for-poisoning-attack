...
def check(f):
    ...
    if f == 0:  # se il flag è 0 aggiorno p
        ...
        result_df = pd.read_csv("result_SVM.csv", sep=",")
...
def main():
    ...
    dataframe = pd.read_csv("data_2000apk.csv", sep=";")
    ...
    print(f'Len malware = {len(malware)}')
    ...
    for k in range(len(malware)):  # eseguo un loop sul numero di malware presenti nella lista
        ...
        for i in range(len(benign)):  # eseguo un loop sul numero di apk benigni presenti nella lista
            ...
            while r <= delta and m < apks_num:
                ...
                # print(f'{controller[r_num]} and {sample[r_num]}')
                ...
                if controller[r_num] == 1 and sample[r_num] == 0:
                    ...
                    # arr_str = f'{sample[0]}'
                    # if malware[k].tolist() in test_set.tolist():
                    # sys.exit()

                    # if malware[k].tolist() not in test_set.tolist():
                    # for element in sample:
                    # arr_str += f', {element}'
                    # print(f'arr_str = {arr_str}')
                    # file.write(f'{arr_str}, ?\n')
                    # test_set = np.append(test_set, malware[k], axis=0)
                    # break
                    # else:
                    # break
